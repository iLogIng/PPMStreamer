...
void test1()
{
    ...
    ppm::PPMStream ppms("./picture.ppm", scale, w, h, colors);
    ...
    for(int y = 0; y < scale * h; ++y)
    {
        for(int x = 0; x < scale * w; ++x)
        {
            ...
            {
                // ppms.write_pixel({0xFF, 0x00, 0x00});
                ppms.write_pixel(x, y, {0xFF, 0x00, 0x00});
            }
            else
            {
                ppms.write_pixel(x, y, {0x00, 0x00, 0x00});
            }
        }
    }
    ...
    std::cout << "Generate ./picture.ppm" << std::endl;
}
...
void test2()
{
    ...
    for(size_t frame = 0; frame < total_frame; ++frame)
    {
        ...
        ppms.open(output_path, scale, w, h, colors, ppm::RGB::black(), ppm::OpenMode::Rewrite);
        PPMStream::PPMDrawer& drawer = ppms.get_drawer();
    ...
    for(size_t frame = start_frame; frame < end_frame; ++frame)
    {
        ...
        for(int y = 0; y < height; ++y)
        {
            for(int x = 0; x < width; ++x)
            {
                ...
                i = vec2{};
                for(; i.y()++ < 8.0; o += ( sin( v.xyyx() ) + 1.0 ) * abs(v.x() - v.y()))
                {
                    v += cos(v.yx() * i.y() + i + t) / i.y() + 0.7f;
                }
...
void multi_thread_test2()
{
    ...
    for(size_t i = 0; i < Nthrd; ++i)
    {
        thrds.emplace_back(
            ...
            fps,
            i * piece, min((i + 1) * piece, total_frame),
            ppm_path_format
        );
    }
...
void test3()
{
    ...
    std::string output_path = "./output-picture/picture.ppm";
    const int scale = 120;
    const int width = 16 * scale;
    const int height = 9 * scale;
    const int colors = 255;

    PPMStream ppms(output_path, width, height, colors);

    for(size_t y = 0; y < height; ++y)
    {
        for(size_t x = 0; x < width; ++x)
        {
            ppms.write_pixel(x, y, {0xAA, 0x00, 0x00});
        }
    }
...
void test2_simple_pattern()
{
    ...
    const char* output_path = "./output-picture/test_pattern.ppm";
    const int width = 800;
...
void test4()
{
    ...
    PPMStream stream(output_path, width, height, colors, ppm::RGB(0xAA, 0x00, 0x00));

    PPMStream::PPMDrawer& drawer = stream.get_drawer();
...
// ================================================================
    ...
    for(int i = 0; i < N; ++i)
    {
        drawer.draw_line({3 * N, i + N * 2}, {4 * N, i + N * 2 + N / 2}, ppm::RGB::black());
    }

    stream.close();
    std::cout << "Generate to " << output_path << std::endl;
